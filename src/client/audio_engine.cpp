...
bool AudioEngine::init() {
    ...
    // loadSound(static_cast<uint32_t>(shared::SoundId::FOOTSTEP_1), "assets/sounds/oof.mp3");
    // loadSound(static_cast<uint32_t>(shared::SoundId::FOOTSTEP_2), "assets/sounds/oof.mp3");
    // loadSound(static_cast<uint32_t>(shared::SoundId::FOOTSTEP_3), "assets/sounds/footstep_3.wav");
    // loadSound(static_cast<uint32_t>(shared::SoundId::FOOTSTEP_4), "assets/sounds/footstep_4.wav");
    // Section ambients — replace paths with your actual files
...
void AudioEngine::updateEmitter(uint32_t entityId,
    ...
    for (uint8_t i = 0; i < emitter.layerCount; i++) {
        ...
        // start the layer if not already running — starts silent, fade handles volume
...
void AudioEngine::stopLayer(uint32_t entityId, uint32_t soundId) {
    ...
    if (layerVolumes_.count(entityId))
        layerVolumes_[entityId].erase(soundId);
}
