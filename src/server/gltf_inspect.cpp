...
void printCamera(const aiCamera* cam, unsigned i,
                 const shared::ParsedModel& parsed) {
  std::printf("camera[%u] \"%s\" hfov=%g rad aspect=%g near=%g far=%g\n", i,
              cam->mName.C_Str(), cam->mHorizontalFOV, cam->mAspect,
...
void printMaterialProperties(const aiMaterial* mat) {
  for (unsigned p = 0; p < mat->mNumProperties; ++p) {
    ...
    if (*tex) {
      std::printf("  prop \"%s\" [%s/%u] (%s,len=%u): ", prop->mKey.C_Str(),
                  tex, prop->mIndex, propertyTypeName(prop->mType),
                  prop->mDataLength);
    } else {
