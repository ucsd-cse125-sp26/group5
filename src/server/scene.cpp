...
void spawnStaticEntities(ServerGame& game, const std::vector<StaticEntityDesc>& descs) {
  for (auto& desc : descs) {
    ...
    game.registry.emplace<shared::Position>(entity, desc.x, desc.y, desc.z, 1.0f, 0.0f, 0.0f, 0.0f);
    if (desc.render) {
      game.registry.emplace<shared::RenderInfo>(entity, desc.modelName, desc.scale);
    }
    JPH::BodyID bodyId;
    if (desc.meshPath.empty()) {
      ...
      JPH::BodyCreationSettings settings(shape, JPH::RVec3(desc.x, desc.y, desc.z), JPH::Quat::sIdentity(),
                                         JPH::EMotionType::Static, Layers::NON_MOVING);
      JPH::Body* body = bi.CreateBody(settings);
      bi.AddBody(body->GetID(), JPH::EActivation::DontActivate);
      bodyId = body->GetID();
    } else {
      bodyId = game.physics.createMeshBody(desc.meshPath, desc.x, desc.y, desc.z, desc.scale);
    }

    game.registry.emplace<shared::PhysicsBody>(entity, bodyId.GetIndexAndSequenceNumber());
  }
}