...
// Grid uses cells [0, gridW) x [0, gridH). wallMask bit (gy * gridW + gx) == 1 => blocked.
// Returns false if move rejected; optional badMovePenaltyMs added to puzzle elapsed time.
bool StepMemorySpirit(ServerGame& game, uint32_t puzzleEntityId, entt::entity spiritEnt,
                      ...
                      uint64_t wallMask, uint32_t badMovePenaltyMs = 0);

// Assigns one direction per player slot from padOrder (e.g. shuffled). No-op if puzzle not INPROGRESS.
...
// Winter maze solved: puzzle FINISHED, winter section completed, section progress bumped.
void CollectMazeFragment(ServerGame& game);

// True if a WINTER SectionController exists and is unlocked (puzzle id may legitimately be 0).
bool HasUnlockedWinterSection(const ServerGame& game);

// Winter maze puzzle entity id (shared::Entity id), or 0 if no unlocked winter section.
uint32_t GetWinterPuzzleNumericId(ServerGame& game);

// Sets MazePadBinding from each client's join slot so UP = their maze FPV facing (same as computeCamera).
void ClaimPadsForActivePlayers(ServerGame& game, uint32_t puzzleEntityId);

// Per-tick: shared spirit uses Jolt velocity from UP; direction = that client's join-slot facing (FPV).
// G = collect fragment.
